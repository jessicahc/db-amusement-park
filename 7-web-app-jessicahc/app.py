...
def read_post():
    """
    Route for POST requests to the read page.
    Sorts reviews based on selected sort. 
    """
    filter_reviews = request.form['filter reviews']
    # by ride, rating, user, newest posts, oldest posts
    if (filter_reviews == "Six Flags Great Adventure - Kingda Ka"):
        docs = db.exampleapp.find({ride: "Six Flags Great Adventure - Kingda Ka"}).sort("created_at", -1)
    elif (filter_reviews == "Six Flags Great Adventure - El Toro"):
        docs = db.exampleapp.find({ride: "Six Flags Great Adventure - El Toro"}).sort("created_at", -1)
    return render_template('read.html', docs=docs) # render the read template
...
def edit(mongoid):
    ...
    """
    doc = db.exampleapp.find_one({"_id": ObjectId(mongoid)})
    return render_template('edit_post.html', mongoid=mongoid, doc=doc) # render the edit template
...
def edit_post(mongoid):
    ...
    ride_rating = request.form['ride_rating']
    ride_review  = request.form['ride_review']
    ...
    return redirect(url_for('submitted')) # tell the browser to make a request for the /edited route
